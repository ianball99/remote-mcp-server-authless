...
- [ ] **3 Apr 2026 — Eliminate redundant `get_itinerary` call before mutations on TripPage** — The chatbot always calls `get_itinerary` before any mutation tool (e.g. `update_itinerary`, `upload_created_html_itinerary_document`) purely to obtain the `vamoos_id`. However, `TripPage.jsx` already fetches and holds `tripMeta.vamoos_id` in React state at page load. Passing it via `initialSystemContext` to `ChatPanel` would let the chatbot skip this round-trip entirely on the first exchange. The system prompt's fallback rule ("if you don't have the vamoos_id, call get_itinerary first") can remain for edge cases. Post-mutation `get_itinerary` calls (to get full current data for HTML regeneration) are separate and may still be justified.
